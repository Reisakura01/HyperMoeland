...
internal static class AutoStart
{
    ...
    public const string AppName = "Islora";

    /// <summary>清单里 StartupTask 的 TaskId，必须与 AppxManifest 完全一致。</summary>
    private const string StartupTaskId = "IsloraStartup";
    ...
    private static void SetRegistry(bool enabled)
    {
        try
        {
            ...
            // 清理旧版（HyperMoeland 时期）留下的自启项：
            // 否则改名后会出现两条自启记录，登录时同时拉起新旧两个岛
            k.DeleteValue("HyperMoeland", false);
        }
        catch { }
    }
