; ============================================================================
;  HyperMoeland — Inno Setup 安装脚本
;
;  编译（推荐用包装脚本，会自动传入版本号与暂存目录）：
;      pwsh -File packaging/installer/Build-Installer.ps1
;  或直接调用编译器：
;      ISCC.exe /DAppVersion=1.3.0 /DStagingDir=..\..\dist packaging\installer\HyperMoeland.iss
;
;  设计要点：
;    * 每位用户安装（PrivilegesRequired=lowest），装到 %LOCALAPPDATA%\Programs，
;      不需要管理员权限，也不会弹 UAC——与「应用把设置写在 %LOCALAPPDATA%\HyperMoeland」一致。
...
#define AppName "HyperMoeland"
#define AppPublisher "MoeOrigin Team"
#define AppURL "https://github.com/Reisakura01/HyperMoeland"
#define AppExeName "HyperMoeland.exe"
...
SetupIconFile=..\..\HyperMoeland\App.ico
Compression=lzma2/max
SolidCompression=yes
WizardStyle=modern
ArchitecturesAllowed=x64compatible
ArchitecturesInstallIn64BitMode=x64compatible
; HyperMoeland 使用 WPF + 云母（Mica），最低要求 Windows 11（build 22000）
...
[CustomMessages]
chinese.RuntimeMissing=未检测到 .NET 10 桌面运行时（Microsoft.WindowsDesktop.App 10.x）。%n%nHyperMoeland 是框架依赖应用，需要它才能启动。%n请先安装（选「Desktop Runtime」的 x64 版本）：%n%1%n%n是否仍要继续安装？
english.RuntimeMissing=.NET 10 Desktop Runtime (Microsoft.WindowsDesktop.App 10.x) was not found.%n%nHyperMoeland is framework-dependent and needs it to run.%nPlease install the x64 "Desktop Runtime" first:%n%1%n%nContinue the installation anyway?
...
Source: "..\..\HyperMoeland\App.ico"; DestDir: "{app}\identity"; Flags: ignoreversion
